...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: tournament } = await supabase
      .from('tournaments').select('status, created_by').eq('id', tournament_id).single();
    ...
    if (tournament.created_by === userId)
      return NextResponse.json({ error: 'Creator cannot leave — cancel instead' }, { status: 400 });

    await supabase.from('tournament_participants')
      .delete().eq('tournament_id', tournament_id).eq('user_id', userId);
